    ...
    with(as.list(p),{
      ...
      if (t >= thermal_onset_time) {
        ...
        if (current_SST < SST_threshold) {
          ...
          beta_scaling_factor <- exp(-z * (current_SST - SST_threshold))
          gamma_scaling_factor <- exp(-e * (current_SST - SST_threshold))
  ...
  for(i in 1:length(sites)){
    ...
    lower_bounds.tiss = c(0, 0)  # Lower bounds for zeta and eta
    upper_bounds.tiss = c(1, 1)          # Upper bounds for zeta and eta
