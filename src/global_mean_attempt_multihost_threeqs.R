...
SIR.multi <- function(t, y, p) {
  ...
  with(as.list(p), {
    ...
    dS.LS <- -b.LS * S.LS * P * (K.LS / N.LS)^q.LS
    dI.LS <-  b.LS * S.LS * P * (K.LS / N.LS)^q.LS - g.LS * I.LS
    dR.LS <-  g.LS * I.LS
    
    dS.MS <- -b.MS * S.MS * P * (K.MS / N.MS)^q.MS
    dI.MS <-  b.MS * S.MS * P * (K.MS / N.MS)^q.MS - g.MS * I.MS
    dR.MS <-  g.MS * I.MS
    
    dS.HS <- -b.HS * S.HS * P * (K.HS / N.HS)^q.HS
    dI.HS <-  b.HS * S.HS * P * (K.HS / N.HS)^q.HS - g.HS * I.HS
    dR.HS <-  g.HS * I.HS
...
result_global <- DEoptim(
  ...
  lower          = c(0,   0,   0,   0,   0,   0,   0.5, 0.5, 0.5),
  upper          = c(4,   4,   4,   4,   4,   4,   1.0, 1.0, 1.0),
...
q.LS.global  <- result_global$optim$bestmem[7]
q.MS.global  <- result_global$optim$bestmem[8]
q.HS.global  <- result_global$optim$bestmem[9]
...
cat(sprintf("  q_LS: %.4f   q_MS: %.4f   q_HS: %.4f\n",
...
p_global_fit <- ggplot(global_sims %>% filter(Compartment == "Dead"),
  ...
  labs(x     = "Day of outbreak",
       ...
       title = sprintf(
         "Global multi-host SIR fit (q_LS=%.3f, q_MS=%.3f, q_HS=%.3f)\nβ_LS=%.4f γ_LS=%.4f | β_MS=%.4f γ_MS=%.4f | β_HS=%.4f γ_HS=%.4f",
         q.LS.global, q.MS.global, q.HS.global,
