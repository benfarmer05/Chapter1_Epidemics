  ...
  panel_e_mid_ws1 <- readRDS(here("output", "panel_e_data_mid_workspace_1.rds"))
  panel_e_mid_ws2 <- readRDS(here("output", "panel_e_data_mid_workspace_2.rds"))
  
  # Load observed data (assuming it's the same for both)
  obs_e_ws1 <- readRDS(here("output", "panel_e_obs_workspace_1.rds"))
  obs_e_mid_ws1 <- readRDS(here("output", "panel_e_obs_mid_workspace_1.rds"))
  ...
  panel_e_mid_ws2_filtered <- panel_e_mid_ws2 %>%
    filter(Compartment != "Infected")
  ...
  panel_e_mid_ws1$workspace <- "Freq."
  panel_e_mid_ws2_filtered$workspace <- "NL Dens."
  
  # Combine the datasets
  combined_panel_e <- rbind(panel_e_ws1, panel_e_ws2_filtered)
  combined_panel_e_mid <- rbind(panel_e_mid_ws1, panel_e_mid_ws2_filtered)
  ...
  max_midchannel <- max(c(
    max(data_fig3 %>% filter(Site == "Midchannel") %>% pull(tissue), na.rm = TRUE),
    max(combined_panel_e$tissue, na.rm = TRUE)
  ), na.rm = TRUE)
  ...
  p_combined_panel_e_mid <- ggplot(data = combined_panel_e_mid, 
                               aes(x = days.model, y = tissue, 
                                   color = workspace, linetype = workspace)) +
    geom_line(aes(group = interaction(Compartment, workspace)), linewidth = 0.75) +
    geom_point(data = obs_e_mid_ws1, 
               aes(x = days.inf.site, y = tissue, shape = Compartment), 
               color = "black", size = 1.3, inherit.aes = FALSE) +
    scale_x_continuous(limits = c(0, max(combined_panel_e_mid$days.model, na.rm = TRUE))) +
    scale_y_continuous(limits = c(0, max_midchannel)) +  # Set consistent y-axis for offshore panels
    scale_shape_manual(values = c("Susceptible" = 16, "Infected" = 17, "Removed" = 15),
                       name = "") +  # Remove "Compartment" title to match your other plots
    scale_color_manual(values = c("Freq." = "black", "NL Dens." = "#228B22"),  # Black and forest green
                       name = "Model",
  ...
  p5.figS6.final = p_combined_panel_e_mid
  ...
  p3.figS6 <- p3.figS6 + scale_y_continuous(limits = c(0, max_midchannel))
  p4.figS6 <- p4.figS6 + scale_y_continuous(limits = c(0, max_midchannel))
  p6.figS6 <- p6.figS6 + scale_y_continuous(limits = c(0, max_midchannel))
  ...
  figS6_final =
    p1.figS6 + p2.figS6 + p3.figS6 + p4.figS6 + p5.figS6.final + p6.figS6 +
    plot_layout(axis_titles = "collect",
                guide = 'collect',
                design = "AB\nCD\nEF") &
    theme(legend.position = "bottom",
  ...
  figS6_final <- figS6_final + plot_annotation(tag_levels = "A") & 
    theme(plot.tag.position = c(1, 1))  # Move labels to top right
  ...
  quartz(h = 5, w = 7.087)
  figS6_final
  quartz.save(file = here("output", "figS6_final.pdf"), type = "pdf")
  ggsave(filename = here("output", "figS6_final.png"), device = "png", width = 7.087, height = 5, dpi = 1200)
  dev.off()
  
  ################################## create new Fig S4, S7 ##################################
  ...
  obs_e_mid_ws1_filtered <- obs_e_mid_ws1 %>%
    filter(Compartment == "Infected")
  panel_e_mid_ws1_filtered <- panel_e_mid_ws1 %>%
    filter(Compartment == "Infected")
  panel_e_mid_ws2_filtered <- panel_e_mid_ws2 %>%
    filter(Compartment == "Infected")
  ...
  panel_e_mid_ws1_filtered$workspace <- "Freq."
  panel_e_mid_ws2_filtered$workspace <- "NL Dens."
  
  # Combine the datasets
  combined_panel_e <- rbind(panel_e_ws1_filtered, panel_e_ws2_filtered)
  combined_panel_e_mid <- rbind(panel_e_mid_ws1_filtered, panel_e_mid_ws2_filtered)
  ...
  max_midchannel <- max(c(
    max(data_fig3 %>% filter(Site == "Midchannel", Compartment == 'Infected') %>% pull(tissue), na.rm = TRUE),
    max(combined_panel_e$tissue, na.rm = TRUE)
  ), na.rm = TRUE)
  ...
  p_combined_panel_e_mid <- ggplot(data = combined_panel_e_mid, 
                               aes(x = days.model, y = tissue, 
                                   color = workspace, linetype = workspace)) +
    geom_line(aes(group = interaction(Compartment, workspace)), linewidth = 0.75) +
    geom_point(data = obs_e_mid_ws1_filtered, 
               aes(x = days.inf.site, y = tissue), 
               color = "black", size = 1.3, shape = 17) +  # Removed shape aesthetic and set fixed shape
    scale_x_continuous(limits = c(0, max(combined_panel_e_mid$days.model, na.rm = TRUE))) +
    scale_y_continuous(limits = c(0, max_midchannel)) +  # Set consistent y-axis for offshore panels
  ...
  p5.figS7.final = p_combined_panel_e_mid
  ...
  p1.figS7 <- p1.figS4 + scale_y_continuous(limits = c(0, max_nearshore))
  p2.figS7 <- p2.figS4 + scale_y_continuous(limits = c(0, max_nearshore))
  ...
  p3.figS7 <- p3.figS7 + scale_y_continuous(limits = c(0, max_midchannel))
  p4.figS7 <- p4.figS7 + scale_y_continuous(limits = c(0, max_midchannel))
  p6.figS7 <- p6.figS7 + scale_y_continuous(limits = c(0, max_midchannel))
  ...
  figS7_final =
    p1.figS7 + p2.figS7 + p3.figS7 + p4.figS7 + p5.figS7.final + p6.figS7 +
    plot_layout(axis_titles = "collect",
                guide = 'collect',
                design = "AB\nCD\nEF") &
    theme(legend.position = "bottom",
  ...
  figS7_final <- figS7_final + plot_annotation(tag_levels = "A") & 
    theme(plot.tag.position = c(1, 1))  # Move labels to top right
  ...
  quartz(h = 5, w = 7.087)
  figS7_final
  quartz.save(file = here("output", "figS7_final.pdf"), type = "pdf")
  ggsave(filename = here("output", "figS7_final.png"), device = "png", width = 7.087, height = 5, dpi = 1200)
  dev.off()
  
  ################################## create new Fig S5, S8 ##################################
  ...
  # Filter out 'Removed' compartment
  obs_e_ws1_filtered <- obs_e_ws1 %>%
    filter(Compartment == "Removed")
  panel_e_ws1_filtered <- panel_e_ws1 %>%
    filter(Compartment == "Removed")
  panel_e_ws2_filtered <- panel_e_ws2 %>%
    filter(Compartment == "Removed")
  ...
  # For panels A&B (nearshore data) - get max from both single and multi-host nearshore data
  max_nearshore <- max(c(
    max(data_fig3 %>% filter(Site == "Nearshore", Compartment == 'Removed') %>% pull(tissue), na.rm = TRUE),
    max(combined_panel_e$tissue, na.rm = TRUE)  # in case panel E data is higher
  ), na.rm = TRUE)
  
  # For panels C/D/E/F (offshore data) - get max from offshore and panel E data
  max_offshore <- max(c(
    max(data_fig3 %>% filter(Site == "Offshore", Compartment == 'Removed') %>% pull(tissue), na.rm = TRUE),
    max(combined_panel_e$tissue, na.rm = TRUE)
  ), na.rm = TRUE)
  
  max_midchannel <- max(c(
    max(data_fig3 %>% filter(Site == "Midchannel", Compartment == 'Removed') %>% pull(tissue), na.rm = TRUE),
    max(combined_panel_e$tissue, na.rm = TRUE)
  ), na.rm = TRUE)
  ...
  p_combined_panel_e <- ggplot(data = combined_panel_e, 
    ...
    geom_point(data = obs_e_ws1_filtered, 
               aes(x = days.inf.site, y = tissue), 
               color = "black", size = 1.3, shape = 15) +  # Removed shape aesthetic and set fixed shape
    scale_x_continuous(limits = c(0, max(combined_panel_e$days.model, na.rm = TRUE))) +
    scale_y_continuous(limits = c(0, max_offshore)) +  # Set consistent y-axis for offshore panels
    # Removed scale_shape_manual entirely since we don't want it in the legend
    scale_color_manual(values = c("Freq." = "black", "NL Dens." = "#228B22"),  # Black and forest green
  ...
  p_combined_panel_e_mid <- ggplot(data = combined_panel_e_mid, 
    ...
    geom_point(data = obs_e_mid_ws1_filtered, 
               ...
               color = "black", size = 1.3, shape = 15) +  # Removed shape aesthetic and set fixed shape
    scale_x_continuous(limits = c(0, max(combined_panel_e_mid$days.model, na.rm = TRUE))) +
    scale_y_continuous(limits = c(0, max_midchannel)) +  # Set consistent y-axis for midchannel panels
    # Removed scale_shape_manual entirely since we don't want it in the legend
    scale_color_manual(values = c("Freq." = "black", "NL Dens." = "#228B22"),  # Black and forest green
                       name = "Model",
                       breaks = c("NL Dens.")) +  # Only show NL Dens. in legend
  ...
  p5.figS8.final = p_combined_panel_e_mid
  ...
  p1.figS8 <- p1.figS8 + scale_y_continuous(limits = c(0, max_nearshore))
  p2.figS8 <- p2.figS8 + scale_y_continuous(limits = c(0, max_nearshore))
  ...
  p3.figS8 <- p3.figS8 + scale_y_continuous(limits = c(0, max_midchannel))
  p4.figS8 <- p4.figS8 + scale_y_continuous(limits = c(0, max_midchannel))
  p6.figS8 <- p6.figS8 + scale_y_continuous(limits = c(0, max_midchannel))
  ...
  figS8_final =
    p1.figS8 + p2.figS8 + p3.figS8 + p4.figS8 + p5.figS8.final + p6.figS8 +
    plot_layout(axis_titles = "collect",
                guide = 'collect',
                design = "AB\nCD\nEF") &
    theme(legend.position = "bottom",
  ...
  figS8_final <- figS8_final + plot_annotation(tag_levels = "A") & 
    theme(plot.tag.position = c(1, 1))  # Move labels to top right
  ...
  quartz(h = 5, w = 7.087)
  figS8_final
  quartz.save(file = here("output", "figS8_final.pdf"), type = "pdf")
  ggsave(filename = here("output", "figS8_final.png"), device = "png", width = 7.087, height = 5, dpi = 1200)
  dev.off()
