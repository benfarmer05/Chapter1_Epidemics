...
site_data_list <- lapply(sites, function(s) {
  ...
  N.LS   <- susceptible_ref %>% filter(Site == s, Category == "Low")      %>% pull(tissue_ref) %>% sum()
  N.MS   <- susceptible_ref %>% filter(Site == s, Category == "Moderate") %>% pull(tissue_ref) %>% sum()
...
joint_q     <- FALSE   # TRUE = one shared q; FALSE = separate q.LS, q.MS, q.HS; ignored when fit_q = FALSE
q_val_fixed <- 1.0    # used only when fit_q = FALSE

# --- Gamma toggles ---
fix_g.LS    <- FALSE; g.LS.fixed <- 0.14
fix_g.MS    <- FALSE; g.MS.fixed <- 0.14
...
# NOTES - 17 April 2026 [when weighting by N.group or max R.group. also, capping betas at 0.1]
# - freeing q (with LMS != HS) with community freq-dep. (BUT only considering Nearshore & Offshore); weighting by N.group....result is okay. nothing amazing, and nearshore "pops" way too early
...
SIR.multi <- function(t, y, p) {
  ...
  with(as.list(p), {
    ...
    # #community freq-dep.
    # N0 <- N.LS + N.MS + N.HS
    # contact.LS <- (K^q.LS) * S.LS * P / (N0^q.LS)
    # contact.MS <- (K^q.MS) * S.MS * P / (N0^q.MS)
    # contact.HS <- (K^q.HS) * S.HS * P / (N0^q.HS)
    
    #within-group freq-dep.
    contact.LS <- (K^q.LS) * S.LS * P / (N.LS^q.LS)
...
objective_function_global <- function(params, site_data_list,
                                      ...
                                      g.LS.fixed = NULL, g.MS.fixed = NULL, g.HS.fixed = NULL,
                                      norm_by_max_R = FALSE) {
  
  idx <- 1
  b.LS <- params[idx]; idx <- idx + 1
  g.LS <- if (!is.null(g.LS.fixed)) g.LS.fixed else { v <- params[idx]; idx <- idx + 1; v }
  b.MS <- params[idx]; idx <- idx + 1
  g.MS <- if (!is.null(g.MS.fixed)) g.MS.fixed else { v <- params[idx]; idx <- idx + 1; v }
  b.HS <- params[idx]; idx <- idx + 1
  g.HS <- if (!is.null(g.HS.fixed))  g.HS.fixed  else { v <- params[idx]; idx <- idx + 1; v }
  
  if (!is.null(q_fixed)) {
    q.LS <- q_fixed; q.MS <- q_fixed; q.HS <- q_fixed
  } else if (joint_q) {
    q.LS <- params[idx]; q.MS <- params[idx]; q.HS <- params[idx]; idx <- idx + 1
  } else {
    q.LS <- params[idx]; idx <- idx + 1
    q.MS <- params[idx]; idx <- idx + 1
    q.HS <- params[idx]; idx <- idx + 1
  }
  ...
  for (sd in site_data_list) {
    ...
    norm <- if (norm_by_max_R) {
      max(c(sd$LS.remtiss, sd$MS.remtiss, sd$HS.remtiss), na.rm = TRUE)
    } else {
      sd$N.site
    }
    ...
    site_residual <-
      sum(abs(sim.LS.rem - sd$LS.remtiss)) / sd$N.LS +
      sum(abs(sim.MS.rem - sd$MS.remtiss)) / sd$N.MS +
      sum(abs(sim.HS.rem - sd$HS.remtiss)) / sd$N.HS +
      sum(abs(sim.LS.inf - sd$LS.inftiss)) / sd$N.LS +
      sum(abs(sim.MS.inf - sd$MS.inftiss)) / sd$N.MS +
      sum(abs(sim.HS.inf - sd$HS.inftiss)) / sd$N.HS
    
    # # NOTE - normalize by group-level max R
    # site_residual <-
    #   sum(abs(sim.LS.rem - sd$LS.remtiss)) / max(sd$LS.remtiss) +
    #   sum(abs(sim.MS.rem - sd$MS.remtiss)) / max(sd$MS.remtiss) +
    #   sum(abs(sim.HS.rem  - sd$HS.remtiss))  / max(sd$HS.remtiss)  +
    #   sum(abs(sim.LS.inf - sd$LS.inftiss)) / max(sd$LS.remtiss) +
    #   sum(abs(sim.MS.inf - sd$MS.inftiss)) / max(sd$MS.remtiss) +
...
g.LS.arg   <- if (fix_g.LS) g.LS.fixed else NULL
g.MS.arg   <- if (fix_g.MS) g.MS.fixed else NULL
g.HS.arg   <- if (fix_g.HS) g.HS.fixed  else NULL

lower_base <- c(0)
if (!fix_g.LS) lower_base <- c(lower_base, 0)
lower_base <- c(lower_base, 0)
if (!fix_g.MS) lower_base <- c(lower_base, 0)
lower_base <- c(lower_base, 0)
if (!fix_g.HS) lower_base <- c(lower_base, 0)

upper_base <- c(b_upper)
if (!fix_g.LS) upper_base <- c(upper_base, 4)
upper_base <- c(upper_base, b_upper)
if (!fix_g.MS) upper_base <- c(upper_base, 4)
upper_base <- c(upper_base, b_upper)
...
if (fit_q) {
  if (joint_q) {
    cat(sprintf("Fitting beta, %sAND shared q...\n",
                if (!fix_g.LS || !fix_g.MS || !fix_g.HS) "gamma, " else ""))
    result_global <- DEoptim(
      ...
      q_fixed        = NULL, joint_q = TRUE,
      g.LS.fixed     = g.LS.arg, g.MS.fixed = g.MS.arg, g.HS.fixed = g.HS.arg,
      norm_by_max_R  = norm_by_max_R,
      control        = control
    )
    q.LS.global <- tail(result_global$optim$bestmem, 1)
    q.MS.global <- q.LS.global
    q.HS.global <- q.LS.global
  } else {
    cat(sprintf("Fitting beta, %sAND separate q.LS / q.MS / q.HS...\n",
                if (!fix_g.LS || !fix_g.MS || !fix_g.HS) "gamma, " else ""))
    result_global <- DEoptim(
      fn             = objective_function_global,
      lower          = c(lower_base, 0.5, 0.5, 0.5),
      upper          = c(upper_base, 1.0, 1.0, 1.0),
      site_data_list = site_data_list_fit,
    ...
    q.LS.global <- tail(result_global$optim$bestmem, 3)[1]
    q.MS.global <- tail(result_global$optim$bestmem, 3)[2]
    q.HS.global <- tail(result_global$optim$bestmem, 1)
  }
} else {
  cat(sprintf("Fitting beta, %swith q fixed at %.2f...\n",
              if (!fix_g.LS || !fix_g.MS || !fix_g.HS) "gamma, " else "", q_val_fixed))
  result_global <- DEoptim(
    fn             = objective_function_global,
    lower          = lower_base,
    upper          = upper_base,
  ...
  q.LS.global <- q_val_fixed
  q.MS.global <- q_val_fixed
...
idx <- 1
b.LS.global <- res[idx]; idx <- idx + 1
g.LS.global <- if (fix_g.LS) g.LS.fixed else { v <- res[idx]; idx <- idx + 1; v }
b.MS.global <- res[idx]; idx <- idx + 1
g.MS.global <- if (fix_g.MS) g.MS.fixed else { v <- res[idx]; idx <- idx + 1; v }
b.HS.global <- res[idx]; idx <- idx + 1
g.HS.global <- if (fix_g.HS) g.HS.fixed  else { v <- res[idx]; idx <- idx + 1; v }
if (fit_q) {
  q.LS.global <- res[idx]; idx <- idx + 1
  if (joint_q) {
    q.MS.global <- q.LS.global
    q.HS.global <- q.LS.global
  } else {
    q.MS.global <- res[idx]; idx <- idx + 1
    q.HS.global <- res[idx]; idx <- idx + 1
  }
}
...
cat(sprintf("  q.LS: %.4f   q.MS: %.4f   q.HS: %.4f  (%s)\n",
            q.LS.global, q.MS.global, q.HS.global,
...
global_sims <- lapply(site_data_list, function(sd) {
  data.frame(ode(
    ...
    parms = list(b.LS = b.LS.global, g.LS = g.LS.global,
                 ...
                 N.LS = sd$N.LS, N.MS = sd$N.MS, N.HS = sd$N.HS,
                 K     = K,
                 q.LS = q.LS.global, q.MS = q.MS.global, q.HS = q.HS.global)
  )) %>%
    select(-any_of("P")) %>%
    mutate(
      site      = sd$site,
...
p_global_fit <- ggplot(global_sims %>% filter(Compartment == "Dead"),
  ...
  labs(x     = "Day of outbreak",
       ...
       title = sprintf(
         "Global multi-host SIR fit (q.LS=%.3f q.MS=%.3f q.HS=%.3f, %s)\nβ_LS=%.4f γ_LS=%.4f | β_MS=%.4f γ_MS=%.4f | β_HS=%.4f γ_HS=%.4f",
         q.LS.global, q.MS.global, q.HS.global,
         if (!fit_q) "fixed" else if (joint_q) "fitted jointly" else "fitted separately",
