  ...
  top_seed = 77 #ensures stability if you want to get exact (random) set of sub-seeds each run
  set.seed(top_seed)
  iter_seeds  <- sample.int(.Machine$integer.max, size = N_ITER, replace = FALSE) #randomly sample sub-seeds based on top seed
  ...
  ################################## Check prior run completion ##################################
  
  # function determining which prior iterations were already completed
  completed_iters <- function(dir) {
    files <- list.files(dir, pattern = "^iter_(\\d+)\\.rds$", full.names = FALSE)
    as.integer(sub("iter_(\\d+)\\.rds", "\\1", files))
  }
  
  # follow RESUME Boolean
  if (!RESUME) {
    old_files <- list.files(OUTPUT_DIR, pattern = "^iter_\\d+\\.rds$", full.names = TRUE)
    if (length(old_files) > 0) {
      file.remove(old_files)
      message("RESUME = FALSE: cleared ", length(old_files), " existing run(s).")
    }
    done        <- integer(0)
    start_iter  <- 1L
  } else {
    done        <- completed_iters(OUTPUT_DIR)
    remaining   <- setdiff(seq_len(N_ITER), done)
    start_iter  <- if (length(remaining) == 0) {
      message("All ", N_ITER, " iterations already complete. Nothing to do.")
      stop("Done.", call. = FALSE)
    } else {
      message("RESUME = TRUE: ", length(done), " iteration(s) already complete. ",
              "Resuming from iteration ", min(remaining), ".")
      min(remaining)
    }
  }
  
  iters_to_run <- setdiff(seq_len(N_ITER), done)
  ...
  SIR.multi = function(t,y,p){
    ...
    with(as.list(p),{
      ...
      #within-group frequency-dependent
  ...
  # for(iter in iters_to_run){
  foreach(iter = iters_to_run,
          .packages = c("deSolve", "DEoptim", "tidyverse", "dplyr"),
          .export = c(
            "N_ITER", "sites", "SIR.multi", "obs.model", "susceptible_ref",
            "summary", "SST_sites", "error_eval",
            "lambda", "lambda.LS", "lambda.MS", "lambda.HS",
            "offset.LS", "offset.MS", "offset.HS",
            "alpha_val", "k_val", "DHW.modifier",
            "date_threshold", "SST_threshold_value",
            "OUTPUT_DIR", "iter_seeds", "completed_iters"
          )) %dopar% {
            
            #skip if already completed
            final_path <- file.path(OUTPUT_DIR, sprintf("iter_%04d.rds", iter))
            if (file.exists(final_path)) return(NULL)
            ...
            for(i in 1:length(sites)){
              ...
              message("  Site '", site.loop, "' done.  R0: LS=", round(R0.LS,3),
                      "  MS=", round(R0.MS,3), "  HS=", round(R0.HS,3))
              
            }
            
            # ── Save this iteration atomically ──────────────────────────────────────────
            # Write to a temp file first, then rename — so a partial write is never mistaken
            # for a complete iteration if the script is interrupted mid-save.
            iter_result <- list(
              iteration    = iter,
              seed = iter_seeds[iter],
              params.multi = params.multi,
              my.SIRS.multi = my.SIRS.multi
            )
            tmp_path  <- file.path(OUTPUT_DIR, sprintf("iter_%04d.tmp", iter))
            final_path <- file.path(OUTPUT_DIR, sprintf("iter_%04d.rds", iter))
            saveRDS(iter_result, tmp_path)
            file.rename(tmp_path, final_path)
            
            message("  Saved → ", basename(final_path),
                    "  (", length(completed_iters(OUTPUT_DIR)), "/", N_ITER, " complete)")
  ...
  ################################## Amalgamate results ##################################
  # Run this block any time to load all completed iterations into a single list.
  
  all_runs <- lapply(
    sort(list.files(OUTPUT_DIR, pattern = "^iter_\\d+\\.rds$", full.names = TRUE)),
    readRDS
  )
  names(all_runs) <- sprintf("iter_%04d", sapply(all_runs, `[[`, "iteration"))
  
  message("\nAll done. ", length(all_runs), " iteration(s) loaded into 'all_runs'.")
  
  # Convenience: flat data frame of params across all iterations and sites
  params_df <- bind_rows(lapply(all_runs, function(run) {
    # bind_rows(lapply(names(run$params.multi), function(site) {
    bind_rows(lapply(seq_along(run$params.multi), function(j) {
      site <- if (!is.null(names(run$params.multi))) names(run$params.multi)[j] else sites[j]
      as.data.frame(t(run$params.multi[[site]])) %>%
        mutate(iteration = run$iteration, site = site, .before = 1)
    }))
  }))
  
  message("Flat parameter table available as 'params_df' (",
          nrow(params_df), " rows × ", ncol(params_df), " cols).")